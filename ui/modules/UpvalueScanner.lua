...
local function generateScript(elementIndex) 
    ...
    setClipboard(
        ScriptBuilder.UpvalueMutation({
            SourceRepositoryUrl = oh.Constants.SourceRepositoryUrl,
            SourceBaseUrl = oh.Constants.SourceBaseUrl,
            ScriptPath = (closureScript and getInstancePath(closureScript)) or "nil",
            ClosureName = closure.Name,
            UpvalueIndex = index,
            ClosureConstants = tableToString(currentConstants),
            ElementIndex = dataToString(elementIndex),
            LocalRoot = oh.Constants.IsLocal and oh.Constants.LocalRoot or nil,
        })
    )
end
...
getScriptContext:SetCallback(function()
    if selectedLog then
        local path = ClosureSpy.GetScriptPath(selectedLog.Closure)
        if path then
            setClipboard(path)
        end
    end
